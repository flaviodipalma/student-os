...
export function DayTimeline({
  ...
  const renderItem = (item: TimelineItem) => {
    ...
    if (item.kind === "free") {
      ...
      meta = [
        ...meta,
        state === "completed" && item.session.completedMinutes
          ? `${formatDuration(item.session.completedMinutes)} of ${formatDuration(minutes)} done`
          : null,
        course && <CourseTag key="course" code={course.code} color={course.color} />,
      ]
      badge = (
        <span
          className={cn(
