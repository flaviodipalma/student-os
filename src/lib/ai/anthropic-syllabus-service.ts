...
export class AnthropicSyllabusService implements SyllabusAIService {
  ...
  async extractSyllabusData(text: string, context: SyllabusAIContext): Promise<unknown> {
    ...
    if (response.stop_reason === "refusal") {
      logger.warn("syllabus-ai", "request declined", { category: response.stop_details?.category ?? null })
      throw new SyllabusImportError("ai-failed")
    }
    if (response.stop_reason === "max_tokens" || response.parsed_output == null) {
      logger.warn("syllabus-ai", "incomplete structured output", { stopReason: response.stop_reason })
...
function toSyllabusError(error: unknown): SyllabusImportError {
  ...
  if (error instanceof Anthropic.AuthenticationError || error instanceof Anthropic.PermissionDeniedError) {
    logger.error("syllabus-ai", "authentication failed: check ANTHROPIC_API_KEY", { status: error.status })
  ...
  if (error instanceof Anthropic.APIError) {
    logger.error("syllabus-ai", "API error", { status: error.status, type: error.name })
  ...
  logger.error("syllabus-ai", hasKey ? "request failed before reaching the API" : "no credentials: set ANTHROPIC_API_KEY in .env.local")
